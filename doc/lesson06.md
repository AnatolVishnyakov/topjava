...
> Поменял `users\userMeals` в ключах локализации на `user\meal`. Понадобится при локализации ошибок (сделаем позже)
...
`MealRestController` у нас останется, с ним будем работать позже. 
  ...
  - 1.3.3 добавить локализацию и `jsp:include` в `mealForm.jsp / userMeals.jsp`
...
- 3: Если неправильно формируется url относительно контекста приложения (например `/topjava/userMeals/userMeals`), посмотрите на
...
- 6: В MealController общую часть `@RequestMapping(value = "/userMeals")` лучше вынести на уровень класса
