## Запросы к таблице userMeals c разным index (1000 пользователей, у каждого за год по 3 записи в день):
...
begin
 ...
 for i in 1..1000 loop
   ...
   for j in 1..365 loop
     ...
     INSERT INTO userMeals (date_time, description, calories, user_id) VALUES
...
Sort  (cost=28292.17..28292.88 rows=286 width=31) (actual time=203.774..203.782 rows=298 loops=1)
  ...
  ->  Seq Scan on userMeals  (cost=0.00..28280.50 rows=286 width=31) (actual time=0.023..203.714 rows=298 loops=1)
...
Sort  (cost=22858.59..22861.23 rows=1057 width=31) (actual time=147.481..147.481 rows=0 loops=1)
  ...
  ->  Seq Scan on userMeals  (cost=0.00..22805.50 rows=1057 width=31) (actual time=147.474..147.474 rows=0 loops=1)
...
> CREATE INDEX meals_idx ON userMeals (user_id);
...
Sort  (cost=59.88..60.59 rows=286 width=31) (actual time=0.372..0.378 rows=298 loops=1)
  Sort Key: date_time DESC
  Sort Method: quicksort  Memory: 48kB
  ->  Index Scan using meals_user_id_idx on userMeals  (cost=0.43..48.21 rows=286 width=31) (actual time=0.048..0.307 rows=298 loops=1)
...
Sort  (cost=96.02..98.66 rows=1057 width=31) (actual time=0.020..0.020 rows=0 loops=1)
  Sort Key: date_time DESC
  Sort Method: quicksort  Memory: 25kB
  ->  Index Scan using meals_user_id_idx on userMeals  (cost=0.43..42.93 rows=1057 width=31) (actual time=0.013..0.013 rows=0 loops=1)
...
> CREATE INDEX meals_idx ON userMeals (user_id, date_time);
...
```
Index Scan Backward using meals_user_id_date_time_idx on userMeals  (cost=0.43..511.74 rows=286 width=31) (actual time=0.026..0.081 rows=298 loops=1)
...
```
Index Scan Backward using meals_user_id_date_time_idx on userMeals  (cost=0.43..1795.68 rows=1057 width=31) (actual time=0.012..0.012 rows=0 loops=1)
  Index Cond: (user_id = 100003)
