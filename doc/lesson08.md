...
> В `RootControllerTest.testMeals()` сделал проверку через `model().attribute("userMeals", expectedValue)`.
...
`curl -v -H 'Authorization: Basic dXNlckB5YW5kZXgucnU6cGFzc3dvcmQ=' http://localhost:8080/topjava/rest/profile/userMeals`

аналогична

`curl -v --user user@yandex.ru:password http://localhost:8080/topjava/rest/profile/userMeals`
...
- 1: Перевести `userMeals` на `datatables` (`userMeals.jsp`, `MealAjaxController`).
...
- 2: в `topjava.common.js` следует выносите только общие скрипты (cкрипты еды размещайте в  `topjava.userMeals.js`, пользователей в `topjava.users.js`)
